  ...
  part1: (input: string) => {
    ...
    grid.forEach((row, rowIdx) => {
      row.forEach((col, colIdx) => {
        // basically build a copy of the grid with only
        // antennas of the current type
        ...
        // here i'm checking every antenna against the others to see if
        // they're potential antinodes are within the bounds of the grid
        gridOfSameType.forEach((compRow, compRowIdx) => {
          compRow.forEach((compCol, compColIdx) => {
            ...
            const antinode1 = {row: rowIdx - rowDistance, col: colIdx - colDistance};
            const antinode2 = {row: compRowIdx + rowDistance, col: compColIdx + colDistance};
      ...
      row.forEach((col, colIdx) => {
        ...
        gridOfSameType.forEach((compRow, compRowIdx) => {
          compRow.forEach((compCol, compColIdx) => {
            ...
            while (true) {
              const antinode1 = {row: step1.row - rowDistance, col: step1.col - colDistance};
              const antinode2 = {row: step2.row + rowDistance, col: step2.col + colDistance};
